...
public class BTAWorldGenCrystal extends WorldGenerator {
	private boolean useGlowstone;
	
	public BTAWorldGenCrystal(boolean useGlowstone) {
						...
						for (int k = (int) -radius; k <= radius; k++) {
							if (i * i + k * k <= radiusSq) {
								if (rand.nextInt(2) == 0 && useGlowstone) {
									...
									world.setBlock((int) dx + i, (int) dy, (int) dz + k, BTADecoIntegration.amethyst.blockID, 0, 2);
								}
							}
						}
					}
	...
	public boolean doesCrystalIgnoreBlock(int blockID) {
		return blockID == 0 || blockID == Block.lavaStill.blockID || blockID == BTADecoIntegration.amethystShardBlock.blockID;
	}
}