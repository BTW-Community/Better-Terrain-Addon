...
import betterterrain.world.generate.noise.HorizonsNoiseOctaves;
...
public class HorizonsChunkProvider implements IChunkProvider
{
	/** RNG. */
	private Random rand;
	...
	private HorizonsNoiseOctaves soilDepthNoiseGen;
	
	//Cave noise
	private HorizonsNoiseOctaves caveNoiseGenBase;
	...
	public HorizonsChunkProvider(World par1World, long par2, boolean par4) {
		...
		this.soilDepthNoiseGen = new HorizonsNoiseOctaves(this.rand, 4);
		
		this.caveNoiseGenBase = new HorizonsNoiseOctaves(this.rand, 16);
	}
					...
					if (j <= 0 + this.rand.nextInt(5)) {
						...
						if (blockID == 0) {
							...
							if (remainingFillerDepth == -1) {
								...
								else if (DecoIntegration.isDecoInstalled() && remainingFillerDepth == 0 && fillerBlock == DecoIntegration.redSand.blockID) {
									remainingFillerDepth = this.rand.nextInt(4);
