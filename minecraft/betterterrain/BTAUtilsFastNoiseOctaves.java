...
public class BTAUtilsFastNoiseOctaves {
	private BTAUtilsFastNoise[] generators;
	
	public BTAUtilsFastNoiseOctaves(long seed, int numOctaves) {
		Random rand = new Random();
		rand.setSeed(seed);
		
		this.generators = new BTAUtilsFastNoise[numOctaves];
		
		for (int i = 0; i < numOctaves; i++) {
			this.generators[i] = new BTAUtilsFastNoise(rand.nextInt());
		}
	}
