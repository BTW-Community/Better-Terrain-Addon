...
public class BTASurfaceBuilderBadlandsPlateau extends BTASurfaceBuilder {
	...
	public void init(Random rand, long seed) {
		...
		grassNoiseGenSimplex = new BTAUtilsOpenSimplexOctaves(metaRand.nextLong(), 2);
	}
	...
	protected int[] getSurfaceBlock(int i, int j, int k, int surfaceJ, int soilDepth, SurfaceType surfaceType, int seaLevel, Random rand, BTAWorldConfigurationInfo generatorInfo, WorldType worldType) {
		...
		if (useGrass && BTADecoIntegration.isDecoInstalled() && worldType.isDeco() && surfaceType == SurfaceType.TOP && j >= 95) {
			return new int[] {Block.grass.blockID, 0};
		}
		else {
			int blockID = BTADecoIntegration.terracotta.blockID;
			int metadata = 0;
			
			if (blockID == BTADecoIntegration.terracotta.blockID && metaLocations[j & 15] != -1) {
				if (j < 95 || (j < 127 && j != surfaceJ)) {
					blockID = BTADecoIntegration.stainedTerracotta.blockID;
