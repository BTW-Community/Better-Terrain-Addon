...
public class BTWGWorldGenDecoFlowers extends WorldGenerator
{
	/** The ID of the plant block used in this plant generator. */
	private int plantBlockId;
	private int plantMetadata;

	public BTWGWorldGenDecoFlowers()
	{
		plantBlockId = -1;
		plantMetadata = -1;
	}

	public BTWGWorldGenDecoFlowers(int par1)
	{
	...
	public boolean generate(World par1World, Random rand, int par3, int par4, int par5)
	{
		...
		if (plantBlockId == -1) {
			...
			if (i < 16) {
				plantID = BTWGDecoIntegration.flower.blockID;
				plantMetadata = i;
			}
			else if (i < 17) {
				plantID = BTWGDecoIntegration.flower2.blockID;
				plantMetadata = i - 16;
			}
			else {
				plantID = BTWGDecoIntegration.tulip.blockID;
				plantMetadata = i - 17;
			}
		}
