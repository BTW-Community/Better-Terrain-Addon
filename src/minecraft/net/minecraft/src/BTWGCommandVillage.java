...
public class BTWGCommandVillage extends CommandBase {
	...
	public void processCommand(ICommandSender var1, String[] var2) {
		...
		BTWGChunkProvider btwgProvider = null;
		
		try {
			Field chunkProviderField = (ChunkProviderServer.class.getDeclaredField("currentChunkProvider"));
			chunkProviderField.setAccessible(true);
			
			btwgProvider = (BTWGChunkProvider) chunkProviderField.get(provider);
		...
		if (MinecraftServer.getServer().worldServers[0].getWorldChunkManager().areBiomesViable(coords.posX, coords.posZ, 0, BTWGBiomeConfiguration.getVillageBiomes())) {
